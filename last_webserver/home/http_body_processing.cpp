...
{
    std::cout << "Processing multipart data, size: " << data.size() << std::endl;
...
void send_response(int fd, ChunkedClientInfo &client)
{
    ...
    if (client.request_obj.mthod == "content_length")
    {
        ...
        sendErrorResponse(fd, 413, "Request Entity Too Large", "error_page/413.html");
        return;
    }
    ...
    {
        std::string fullPath;
        std::cout << "====> Processing DELETE request for: " << client.request_obj.uri << std::endl;
        ...
        std::cout << "fullname : " << fullPath << std::endl;
        struct stat pathStat;
        if (stat(fullPath.c_str(), &pathStat) != 0)
        {
            sendErrorResponse(fd, 404, "Not Found", "not_found.html");
            return;
        }
        ...
        if (status == 0)
        {
           sendErrorResponse(fd, 200, "OK", "error_page/200.html"); // Successfully deleted
            return;
        }
        else
        {
            sendErrorResponse(fd, 500, "Internal Server Error", "error_page/500.html"); // Internal error
            return;
        }
    ...
    else
    {
        ...
        response_post("error_page/405.html", fd, response);
    }
}
