...
std::string get_buffer(std::string buffer, ssize_t bytes_read, ssize_t &size_chunk, ChunkedClientInfo &client)
{
    ...
    if (size_chunk > 0)
    {
        std::cout << "====> Reading chunked data, size_chunk: " << size_chunk << std::endl;
        if ((ssize_t)buffer.size() >= size_chunk)
        {
            std::cout << "====> Full chunk available, size_chunk: " << size_chunk << std::endl;
            ...
            if (stor_data.find("\r\n") == std::string::npos)
            {
                std::cout << "====> Incomplete chunk, waiting for more data..." << std::endl;
...
bool transfer_encoding_chunked(int fd, ChunkedClientInfo &client)
{
    ...
    if (bytes_read > 0)
    {
        ...
        write_body_to_file(client, data.c_str(), data.size());
         ...
         std::cout << "size of read file: "  << client.bytes_read << std::endl;
            std::cout << "size of content_length is : "  << client.request_obj.server.client_max_body_size<<  std::endl;
         if(client.bytes_read > client.request_obj.server.client_max_body_size)
         {
            ...
            std::cout << "size of file bigger that client_max_body_size " << std::endl;
            client.upload_state = 2;
            client.request_obj.path = "error_page/413.html";
            return true;
...
{
   // client.request_obj.server.client_max_body_size = global_obj[socket_fd_to_default_index].server.client_max_body_size;
    ...
    case 0: // Reading headers
        ...
        { 
            ...
            {
                sendErrorResponse(fd, 400, "Bad Request", "error_page/400.html");
                client.is_active = false;
            }
        }
        break;
